...
if "%1" EQU "-arm64" && shift && goto CheckOpts
...
%MSBUILD% "%D%launcher\launcher.wixproj" /p:Platform=arm64 %CERTOPTS% /p:ReleaseUri=%RELEASE_URI%
if errorlevel 1 exit /B
...
:Help
echo buildrelease.bat [--out DIR] [-arm64] [--certificate CERTNAME] [--build] [--pgo COMMAND]
...
echo    --pgo               Specify PGO command for installers
